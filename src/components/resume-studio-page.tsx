...
function buildScoreMarkdown(jobTitle: string, result: ScoreResult) {
  const lines = [
    ...
    "## 当前薄弱点",
    "",
    ...(result.weakPoints.length ? result.weakPoints.map((item) => `- ${item}`) : ["- 暂无"]),
    "",
    "## 建议补的短项目",
    "",
  ];
...
export function ResumeStudioPage() {
  ...
  async function runTailor() {
    ...
    try {
      ...
      const body = (await res.json()) as { tailored?: TailorResult; error?: string };
  ...
  async function runCoverLetter() {
    ...
    try {
      ...
      const body = (await res.json()) as { coverLetter?: string; error?: string };
      if (!res.ok || !body.coverLetter) throw new Error(body.error ?? "开场白生成失败");
  ...
  async function importResumeFile(file: File) {
    ...
    const body = (await res.json()) as { rawMarkdown?: string; message?: string; error?: string };
  ...
  function renderResultPanel() {
    if (activeResultTab === "score") {
      if (!scoreResult) {
        return (
          <div className={`flex min-h-0 flex-1 overflow-hidden ${resultViewportClass}`}>
            <div
              className={`h-full w-full rounded-[1.5rem] bg-sky-50/70 p-5 text-sm text-muted-foreground ${panelScrollClass}`}
            >
              先点一次“AI 评估”，这里会显示匹配分数、已覆盖、缺失点、一句话判断、当前薄弱点和建议补充短项目。
            </div>
          </div>
        );
      }
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">按阅读顺序往下看，每段都用更平直的横向排法。</p>
                </div>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      downloadMarkdown(
                        ...
                        buildScoreMarkdown(selectedJob?.title ?? "目标岗位", scoreResult),
                      )
                    }
                  >
              ...
              <div className={`${resultCardClass} space-y-3`}>
                <div className="flex flex-col gap-3 border-b border-slate-100 pb-3 sm:flex-row sm:items-center">
                  <div className="w-24 shrink-0 text-sm font-semibold text-slate-900">匹配分数</div>
                  <div className="flex flex-wrap items-end gap-2">
                    ...
                    <span className="pb-1 text-sm text-muted-foreground">基于岗位要求与当前简历的整体贴合度</span>
                  </div>
                </div>
                <div className="flex flex-col gap-3 border-b border-slate-100 pb-3 sm:flex-row sm:items-start">
                  <div className="w-24 shrink-0 text-sm font-semibold text-slate-900">已覆盖</div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
                  <div className="w-24 shrink-0 text-sm font-semibold text-slate-900">缺失点</div>
                  <div className="flex-1 space-y-2">
              ...
              <div className={resultCardClass}>
                <p className="text-sm font-semibold text-slate-900">一句话判断</p>
                <p className="mt-2 text-sm leading-7 text-foreground">{scoreResult.summary}</p>
              </div>

              <div className={resultCardClass}>
                <p className="text-sm font-semibold text-slate-900">当前薄弱点</p>
              ...
              <div className={resultCardClass}>
                <p className="text-sm font-semibold text-slate-900">建议补充短项目</p>
        ...
        <div className={`flex min-h-0 flex-1 overflow-hidden ${resultViewportClass}`}>
          <div className="flex h-full w-full min-h-0 flex-col">
            ...
            <div className="mt-4 min-h-0 flex-1">
              ...
              >
                <p className="whitespace-pre-wrap">{tailorResult.fullMarkdown}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                ...
                variant="outline"
                className="rounded-full"
                onClick={() => downloadMarkdown("opening-message.md", coverLetter)}
              >
    ...
    <div className="grid gap-6 xl:items-stretch xl:grid-cols-[0.9fr_1.05fr_0.95fr]">
      <section className={studioColumnClass}>
        <div className="space-y-2">
          <h2 className="text-xl font-semibold tracking-[-0.03em]">目标岗位</h2>
            ...
            <div className="flex h-full min-h-0 flex-col rounded-[1.5rem] bg-sky-50/80 p-4">
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-xs text-muted-foreground">
                <span className="rounded-full border border-sky-100 bg-white px-2.5 py-1">
                  {pickText(selectedJob.companyInfo, ["city", "location"]) || "未知城市"}
                </span>
                <span className="rounded-full border border-sky-100 bg-white px-2.5 py-1">
                  {pickText(selectedJob.companyInfo, ["experience"]) || "不限经验"}
                </span>
                <span className="rounded-full border border-sky-100 bg-white px-2.5 py-1">
                  {pickText(selectedJob.companyInfo, ["degree"]) || "不限学历"}
                </span>
              </div>
      ...
      <section className={studioColumnClass}>
        <div className="space-y-2">
          <h2 className="text-xl font-semibold tracking-[-0.03em]">简历输入</h2>
          <p className="text-sm text-muted-foreground">
      ...
      <section className={studioColumnClass}>
        <div className="inline-flex flex-wrap gap-1 rounded-full border border-sky-100 bg-sky-50/70 p-1">
          <Button
            ...
            disabled={busyKey !== null}
          >
            {busyKey === "score" && activeResultTab === "score" ? "评估中..." : "AI 评估"}
          </Button>
          <Button
            type="button"
